...
public class ProverbProtocol {
    ...
    private String processGuess(String theInput) {
        if (theInput.equalsIgnoreCase(ANSWER_LIST.get(currentClue))) {
            ...
            return "EXCELENT! " + CLUE_LIST.get(currentClue) + " " + ANSWER_LIST.get(currentClue) + "! " + "Keep playing? (y/n)";
        } else {
            state = Status.SENT_CLUE;
            return "It is supposed to say " + ANSWER_LIST.get(currentClue) + ". " + "Try again! " + CLUE_LIST.get(currentClue);
        }
    }
    ...
    private String processGreeting(String theInput) {
        if (theInput.equalsIgnoreCase("Ready")) {
            ...
            return "You're supposed to say " + "Ready" + "! " + "Try again. " + "Are you ready?";
        }
    }
  ...
  /* Checks wether a given message from the server is supposed to end the game
   *
   * @param fromServer The message from the server
   * @return Wether the message is the the end game
   */
    public static boolean isEndGame(String fromServer) {
        return fromServer.equals(EXIT_MESSAGE);
    }

  /* Process the input and return the interaction accordingly
   *
   * @param theInput The input from the player
   * @return The message for the next interaction
   */
